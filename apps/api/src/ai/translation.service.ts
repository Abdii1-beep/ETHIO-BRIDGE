...
export class TranslationService {
  ...
  private readonly supported = new Set(['en', 'zh', 'zh-CN', 'am', 'om']);
  ...
  private isTranslatableLength(text: string): boolean {
  ...
  private apiKey(): string | undefined {
    return this.config.get<string>('AI_TRANSLATE_API_KEY') || undefined;
  }

  private apiUrl(): string {
    return (
      this.config.get<string>('AI_TRANSLATE_API_URL') ||
  ...
  private async tryApiTranslation(text: string, from: string, to: string): Promise<TranslationResult> {
    ...
    try {
      ...
      const out = json.responseData.translatedText as string;
  ...
  private getFallbackTranslation(text: string, from: string, to: string): string | null {
    ...
    if (to === 'zh') return `[中文] ${text}`;
    if (to === 'am') return `[አማርኛ] ${text}`;
    if (to === 'om') return `[Afaan Oromoo] ${text}`;
